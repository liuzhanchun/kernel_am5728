...
static int vip_port_busy(struct vip_port *port);
...
static int vip_start_streaming(struct vb2_queue *vq, unsigned int count)
{
	...
	/* Make sure mutil ch devide enable first stream on */
	if (port->num_streams_configured == 1)
		enable = 1;
	else {
		if (port->num_streams_mask)
			enable = 0;
		else
			enable = 1;

		port->num_streams_mask |= 0x1 << stream->vfd->index;
	}

	vip_dbg(2, dev, "start_streaming %s: index(%d), mask(0x%x)\n",
		video_device_node_name(stream->vfd),
		stream->vfd->index, port->num_streams_mask);
...
static void vip_stop_streaming(struct vb2_queue *vq)
{
	...
	/* Make sure mutil ch devide disable last stream off */
	if (port->num_streams_configured == 1)
		disable = 1;
	else {
		port->num_streams_mask &= ~(0x1 << stream->vfd->index);
	...
	vip_dbg(2, dev, "stop_streaming %s: index(%d), mask(0x%x)\n",
		video_device_node_name(stream->vfd),
		stream->vfd->index, port->num_streams_mask);

	if (port->subdev && vip_port_busy(port) == 1) {
...
/*
 * vip_port_busy returns the number of stream in the busy state
 * for the specified port.
 * Upon starting the very first stream it will return 0.
 * Upon stopping the very last stream it will return 1 because
 * until vip_stop_streaming() has completed the vb2 queue is still
 * mark as busy.
 *
 * Since this helper is called from both vip_start_streaming and
 * vip_stop_streaming the extra logic is needed.
 */
static int vip_port_busy(struct vip_port *port)
{
	struct vip_stream *stream;
